...
WEBSITE_CHECKS = [
    ("HIGH", "IDOR/ATO — enumerate user IDs in API endpoints", "field-manual → IDOR section"),
    ("HIGH", "Authentication bypass — test all auth flows (login, reset, OAuth)", "field-manual → Auth Bypass section"),
    ("HIGH", "SSRF — find server-side URL fetch params (`url=`, `webhook=`, `redirect=`)", "field-manual → SSRF section"),
    ("HIGH", "Race condition — parallel requests on transactions, coupons, credits", "field-manual → Race Conditions section"),
    ("MED",  "Stored XSS — user input reflected in other users' views", "field-manual → XSS section"),
    ("MED",  "CSRF — state-changing requests without CSRF tokens", "field-manual → CSRF section"),
    ("MED",  "Open redirect — `returnTo`, `next`, `url` params with unvalidated URLs", "field-manual → Open Redirect"),
    ("MED",  "Subdomain takeover — dangling CNAMEs to unclaimed services", "surface-mapping → Phase 8"),
...
OPENSRC_CHECKS = [
    ("HIGH", "Timing side-channel — `===` on HMAC/token/secret comparisons", "field-manual → Timing Side-Channel section"),
    ("HIGH", "JWT/token forgery — `alg:none`, weak secret, claim injection", "field-manual → OIDC/OAuth section"),
    ("HIGH", "Smart contract reentrancy / access control", "field-manual → Smart Contract section"),
    ("HIGH", "SSRF in server-side fetch — user-controlled URL passed to fetch/axios", "field-manual → SSRF section"),
    ("MED",  "Event spoofing — SDK public `trigger()` / `postMessage` without origin check", "field-manual → SDK/Client-Library section"),
    ("MED",  "Open redirect — `new URL(userInput, base)` does NOT prevent open redirect", "field-manual → Open Redirect"),
    ("MED",  "SIWE double-hash / nonce reuse", "field-manual → SIWE section"),
    ("MED",  "Hardcoded secrets in `.env.test` / config files", "manual grep"),
    ("MED",  "Prototype pollution — unsafe `Object.assign` / deep merge on user input", "field-manual → Prototype Pollution"),
    ("LOW",  "Dev breadcrumbs — TODO/FIXME/HACK near security-sensitive code", "grep -rn 'TODO|FIXME|HACK'"),
]

API_CHECKS = [
    ("HIGH", "Auth bypass — test endpoints without Authorization header", "field-manual → Auth Bypass section"),
    ("HIGH", "IDOR — change numeric/UUID IDs in all requests", "field-manual → IDOR section"),
    ("HIGH", "Webhook SSRF — webhook URL field = SSRF vector", "field-manual → SSRF section"),
    ("HIGH", "JWT claim forgery — `x-hasura-role`, `sub`, `iss` manipulation", "field-manual → OIDC/OAuth section"),
    ("MED",  "Endpoint enumeration — undocumented v1/v2 endpoints, hidden routes", "kiterunner scan"),
    ("MED",  "Rate limit bypass — no rate limit on auth, OTP, or sensitive endpoints", "field-manual → rate limiting"),
    ("MED",  "Race condition on batch/parallel operations", "field-manual → Race Conditions section"),
    ("LOW",  "CORS misconfiguration — wildcard on credentialed endpoints", "field-manual → CORS"),
    ("LOW",  "API key in URL — logs exposure of credentials", "manual review"),
]

MOBILE_CHECKS = [
    ("HIGH", "WebView JS injection — `addJavascriptInterface` without origin check", "field-manual → SDK/Client-Library section"),
    ...
    ("MED",  "SDK event spoofing — MiniKit/WalletConnect postMessage without origin check", "field-manual → SDK section"),
...
TECH_CHECKS = {
    "graphql": [
        ("HIGH", "GraphQL IDOR — swap internalId in queries/mutations", "field-manual → GraphQL section"),
        ("MED",  "Introspection enabled — schema leakage, hidden fields/types", "field-manual → GraphQL section"),
        ("MED",  "Batch/alias abuse — 10k mutations to bypass rate limit", "field-manual → GraphQL section"),
    ],
    "nextjs": [
        ("HIGH", "Next.js middleware bypass — check _next/static path for auth bypass", "CVE-2025-29927"),
        ("MED",  "SSRF in getServerSideProps — user-controlled fetch URL", "field-manual → SSRF section"),
        ("MED",  "Server Actions CSRF — test server actions without CSRF token", "field-manual → CSRF section"),
    ],
    "solidity": [
        ("HIGH", "Reentrancy — check all ETH transfer calls", "field-manual → Smart Contract section"),
        ("HIGH", "Signature replay — EIP-712 domain separator, nullifier check", "field-manual → Smart Contract section"),
        ("HIGH", "Access control — `onlyOwner` missing on privileged functions", "field-manual → Smart Contract section"),
        ("MED",  "Front-running — any state-dependent transaction ordering", "field-manual → Smart Contract section"),
    ],
    "jwt": [
        ("HIGH", "alg:none attack — remove signature entirely", "payload-bank → JWT section"),
        ("HIGH", "Weak secret — brute force with hashcat/jwt-cracker", "payload-bank → JWT section"),
        ("MED",  "Claim injection — add `role: admin` or `x-hasura-role: admin`", "field-manual → OIDC section"),
    ],
    "oauth": [
        ("HIGH", "Open redirect in redirect_uri — steal auth code", "field-manual → OIDC/OAuth section"),
        ("HIGH", "CSRF on OAuth flow — missing state parameter check", "field-manual → OIDC/OAuth section"),
        ("MED",  "Token leakage via referrer — access token in URL fragment", "field-manual → OIDC/OAuth section"),
    ],
    "hasura": [
        ("HIGH", "JWT claim forgery — `x-hasura-role: admin` in JWT payload", "payload-bank → Hasura section"),
        ("HIGH", "Admin secret in .env.test — check if reused in staging/prod", "manual check"),
        ("MED",  "Action handler SSRF — Hasura action webhook URL configurable", "field-manual → SSRF section"),
    ],
    "aws": [
        ("HIGH", "SSRF to IMDSv1 — 169.254.169.254/latest/meta-data/iam/security-credentials", "field-manual → SSRF section"),
    ...
    "react": [
        ("MED",  "dangerouslySetInnerHTML with user input — DOM XSS", "field-manual → XSS section"),
